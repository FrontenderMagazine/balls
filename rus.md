...
негативно сказаться на производительности. Кроме того, сферы, созданные таким 
способом, часто выглядят немного грубовато, ведь чтобы сделать сферу 
...
Теперь, когда у нас есть круг, с которым можно дальше работать, мы можем 
стилизировать его в нечто более напоминающее сферу. 

## Добавляем тень

Одним из первых шагов в большинстве уроков по созданию 3D-сфер является 
...
центра (начала) градиента. Затем указывается тип его формы (круг или эллипс). И 
...
В этом примере указаны только два цвета. Это позволяет браузеру предположить что 
у первого координаты 0%, а у второго — 100% и отрисовать переход цвета между 
ними. Если мы хотим чтобы координаты цветов были другими, их можно указать в 
...
Так как мы прописали значение перспективы для контейнера `stage`, в результате 
мы смотрим на круглый элемент `shadow` как бы сверху вниз и видим его в форме 
вытянутого овала. 
...
реалистичным, мы добавим два градиента используя псевдоселектор чтобы казалось 
что свет падает из двух источников. 
...
сам градиент заканчивается примерно по центру мяча. 

Вместо того чтобы создавать новый элемент для подсветки, я использовал 
псевдоселектор `before`. 
...
чтобы наш мяч стал похожим на шар для снукера. 

Чтобы добиться такого впечатления, используем мягкую подсветку снизу как и 
раньше, а вот верхнюю сделаем меньше и чётче. Нам понадобятся два 
псевдоселектора чтобы задать цвет мяча, нижнюю подсветку и блик.
...
Новым для нас в этом коде является псевдоселектор `after`. Он содержит круговой 
градиент, который начинается с почти непрозрачного белого цвета по центру и 
переходит в прозрачный в пометке 24%. Это создаёт эффект белого блика, но чтобы 
сделать его более трёхмерным, мы добавляем свойство `transform`. 

С помощью `transform` сдвигаем блик на 80 пикселей влево и на 90 пикселей вправо, 
и добавляем наклон. эффект наклона вытягивает круг по оси Х, так что блик более 
похож на тот что можно увидеть на гладком шаре. 
...
добавить цифру 8 в разметку, я добавляю её через CSS используя псевдоселектор 
...
псевдоселекторы для создания зрачка и блика. Для элемента `iris` ( *англ.: 
...
больше прозрачности основного цвета и два псевдоселектора чтобы добавить больше 
свечения.
...
Анимация применяется для всего пузырька и его псевдоселеторов. 
...
псевдоселекторов. К примеру, вот текстура теннисного мяча без теней: 
...
демо с Codepen чтобы увидеть глобус в движении. Я установил отображение HTML по 
умолчанию, так как воспроизведение этого примера сильно влияет на 
...
глобуса. Он выглядит потрясающе.
...
Ищете другие примеры создания 3D-объектов? Взгляните на [Zelda — связь с CSS][7] 
и [Воссоздание сцены из обучающего видео игры PORTAL с помощью CSS][8]. 
